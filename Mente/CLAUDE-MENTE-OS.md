...
1. **READ FIRST** ⬜ `Mente/memory/RESUME.md` (created at install) (~5 KB) — the cold-start brief: where we left off +
   next step + flags + pointers. **In ~90% of cases it is ALL you need.**
2. ⛔ **Do NOT read** any large historical file unless a pointer in
   RESUME sends you there explicitly. Reading it "just in case" burns tokens — measured.
...
4. If the owner says "read RESUME" → that file + its pointers.
...
| ⭐ **WHICH REPO this work goes to and who governs it** | declared in `Mente/accounts.tsv` |
...
| `Mente/pieces.tsv` | ⭐ **where each key piece lives** — moving something costs 1 line |
...
|---|---|
| ⭐ **real state + next step** | ⬜ `Mente/memory/RESUME.md` (created at install) |
| any **live number** (battery, gaps, permissions, quotes) | 🤖 ⬜ `Mente/docs/METRICS.md` |
| product version · closed phases · which blocks exist | ⬜ `Mente/memory/RESUME.md` (created at install) §5 |
| open pending items | ⬜ `Mente/memory/PENDING.md` |

---

Related: `PROJECT-RULES.md` (🏢 the project level this router declares) · `Mente/base-rules.md` (🌐 universal) · ⬜ `Mente/memory/RESUME.md` (created at install) (the real state) · `Mente/rules/rule-inheritance.md` (why rules do NOT go here) · `Mente/rules/contract-document.md` (the form this file meets).