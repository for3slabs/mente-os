...
case("① ⬜ no registry → NOT MEASURED, not a ✅",
     r.returncode == 0 and "NOT MEASURED" in r.stdout and "✅" not in r.stdout)
r = run("")
case("② ⬜ an empty registry → NOT MEASURED (a clone starts this way)",
     r.returncode == 0 and "NOT MEASURED" in r.stdout and "✅" not in r.stdout)
...
r = run(GOOD)
case("③ ⭐ a correct row is NOT reported", r.returncode == 0, "exit=%d" % r.returncode)

# ── ACC-REG-002 · a repository that cannot justify itself ───────────────────
ok, r = detects(GOOD.replace("where the work is built", "-"), "ACC-REG-002")
case("④ 🔴 no `por_que_existe` → detected", ok)

# ── ACC-REG-004 · THE ONE THAT MATTERS ──────────────────────────────────────
ok, r = detects(GOOD.replace("\ttaller\t", "\tinventado\t"), "ACC-REG-004")
case("⑤ 🔴 an UNDECLARED role → detected", ok)
case("⑥ ⭐ and it explains that an invisible role lets a retired repo through",
...
case("⑦ ⭐ the %d roles from the TEMPLATE are accepted on their own" % len(declared),
...
case("⑧ ⬜ with no template it says it did NOT measure the roles",
...
case("⑪ 🔴 a row with missing columns → detected", ok)
...
case("⑮ ⭐ a retired repo is NAMED without being a violation",
     r.returncode == 0 and "RETIRED" in r.stdout, "exit=%d" % r.returncode)

# ── the report says what it measured ────────────────────────────────────────
r = run(GOOD)
case("⑯ ⭐ the report says how many repos and how many roles it measured",
...
case("⑰ ⬜ the registry can be declared elsewhere",
...
r = subprocess.run([sys.executable, CHECK], cwd=TREE, capture_output=True,
                   text=True)
case("⑱ ⛔ an unreadable registry does not crash the check",
