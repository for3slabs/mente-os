...
| does everything point to something that exists? | `bin/check-document` |
| does every declared piece still have its file? | `bin/check-structure` |
| do the blocks meet their contract? | `bin/check-block` |
| ⭐ the full verification | `bin/probes/run-all.py` — *the only thing that matters is `failed: 0`* |
